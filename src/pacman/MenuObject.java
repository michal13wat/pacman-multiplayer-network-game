...
public class MenuObject extends GameObject
{
    ...
    private class SpinnerOption extends MenuOption
    {
        ...
        public void addValue(int x)
        {
            ...
            if (valueWrapper.value > rightBound)
            {valueWrapper.value = rightBound;}
            if (valueWrapper.value < leftBound)
            {valueWrapper.value = leftBound;}
        }
        ...
        {
            if (dir == "left")
            {return (valueWrapper.value == leftBound);}
            if (dir == "right")
            {return (valueWrapper.value == rightBound);}
    ...
    public void drawEvent(Graphics2D g)
    {
        ...
        for (int i = 0; i < myOptions.size(); i ++)
        {
            ...
            if (o instanceof SpinnerOption)
            {
                ...
                String s = "";
                if ((so.spinnerFull("left")) || (cursorPos != i))
                {s += " ";}
                else
                {s += "<";}
                s += String.valueOf(so.getValue());
                if ((so.spinnerFull("right")) || (cursorPos != i))
                {s += " ";}
                else
                {s += ">";}
